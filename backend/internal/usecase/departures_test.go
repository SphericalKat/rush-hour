...
func (m *mockTrainRepo) GetStops(_ context.Context, _ string, _ string) ([]train.Stop, error) {
	return nil, nil
}
...
func (m *mockTrainRepo) GetLineStationsWithCoords(_ context.Context, _ string) ([]train.StopWithCoord, error) {
	return nil, nil
}
...
func TestDeparturesChronologicalOrder(t *testing.T) {
	...
	got, err := uc.GetDepartures(context.Background(), 1, nil)
	require.NoError(t, err)
	require.Len(t, got, 2)
	require.Equal(t, "90001", got[0].Number)
	require.Equal(t, "90003", got[1].Number)
}
